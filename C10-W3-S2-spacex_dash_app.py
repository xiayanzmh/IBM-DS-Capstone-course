...
def get_pie_chart(entered_site):
    filtered_df = pd.DataFrame(spacex_df.groupby(['Launch Site','class'])['class'].count()).rename(columns={'class':'count'}).reset_index()
    if entered_site == 'All Sites':
        fig = px.pie(filtered_df[filtered_df['class']==1], values='count', 
        names='Launch Site', 
...
def get_scatter_plot(site,payload):
    ...
    if site == 'All Sites':
        ...
        # return the outcomes piechart for a selected site
        fig = px.scatter(filtered_df[filtered_df['Launch Site']==site], x='Payload Mass (kg)', y='class',
        color="Booster Version Category", 
